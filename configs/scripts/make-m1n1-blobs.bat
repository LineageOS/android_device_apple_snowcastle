...
if not exist "%SYSCFG%" (
    ...
    copy /b m1n1.bin + m1n1-vars-boot.txt + dtb.img + kernel + ramdisk-recovery.img m1n1-recovery.bin
) else (
    copy /b m1n1.bin + m1n1-vars-boot.txt + dtb.img + "%SYSCFG%" + kernel + ramdisk.img m1n1-boot.bin
    copy /b m1n1.bin + m1n1-vars-boot.txt + dtb.img + "%SYSCFG%" + kernel + ramdisk-recovery.img m1n1-recovery.bin
)
